...
impl FromStr for Rucksack {
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		for (index, item) in items.enumerate() {
			let target = if index < len / 2 { Compartment::Left } else { Compartment::Right };
...
fn find_duplicate_item(sack: &Rucksack) -> Option<&char> {
	for item in sack.items() {
		if sack.exists_in_compartment(Compartment::Left, item) && sack.exists_in_compartment(Compartment::Right, item) {
			return Some(item);
		}
	}
	None
